...
6. 先检查“运行前确认”中的字幕来源、模型位置、复核策略、预计工作量和输出位置。估算会结合字幕数或片长、batch size、断点、缓存与夜间复核轮数，显示模型调用量和宽松耗时区间；网络失败重试与低置信内容的额外修复不计入其中。远程模型断开时运行会保持锁定，不会静默改用本地 GPU；来源设置变化后必须重新分析。
...
9. 成片生成后，“成片复核”默认按严重度和共同根因汇总，再下钻到可操作代表样本；报告发现总数、报告保存样本数、已人工复核样本数和去重后的待复核数分别显示，不会把截断样本误报成全部问题。代表样本同时覆盖最严重候选与整片时间轴。“复核”会打开同一时间段的视频帧、上下文音频、波形、前后字幕和编辑区。保存后再运行“继续任务”重新生成。人工时间调整会保存来源锚点，断点恢复和独立终审不会丢失或覆盖已确认内容。
...
12. 已生成内容会集中显示在“交付产物”，可定位 ASS、SRT、字体 MKS、质量报告、渲染预览等已知文件；“最近任务”从输出根目录恢复历史任务并可安全打开对应任务目录。
...
表单和非敏感远程配置由本地前端统一保存在 `%LOCALAPPDATA%\BilingualSubtitlePipeline\frontend-settings.json`，因此切换前端端口后仍会恢复。`.frontend-settings.json.lock` 使用跨进程文件锁保护“读取、合并、原子写入”全过程，多个历史端口同时保存时不会再让表单与远程连接配置互相覆盖。密钥模式只保存可选的密钥路径，不读取或复制私钥内容。密码模式勾选保存后使用当前 Windows 用户的 DPAPI 加密，配置文件和浏览器 `localStorage` 都不保存明文密码；其他 Windows 用户不能解密。密钥认证或已用 DPAPI 保存的密码才能在网页关闭后自动重连；密码模式不勾选保存时，当前网页连接仍可用，但界面会明确提示它不能持久恢复。保存接口只允许 loopback Host，请求体必须是 `application/json` 且不超过 1 MiB；前端也拒绝绑定到非本机地址。

## 系列持久化批量队列

队列状态保存在 `runtime/queue/subtitle-queue.json`，由跨进程文件锁保护并原子替换。队列只串行领取一集，避免多个长片同时争用 GPU、远程模型和输出目录。支持暂停领取、上下移动、取消、失败后重试、移除以及加载已完成剧集进入成片复核。完成状态会区分质量报告发现总数、真正进入复核队列的代表样本数，以及代表样本已全部人工复核但报告仍保留规则告警的状态。
...
“运行前确认”的工作量估算优先使用已抽取的字幕数量；尚未抽取时按视频时长给出明确标记的粗估字幕数。模型调用量分开计算首轮校对/翻译、风格与术语处理、独立终审和夜间复核，并在兼容 checkpoint 与对应缓存已经完成时扣除可复用工作。耗时范围按所选模型体量保守放宽，只用于排期，不是服务承诺；网络重试、模型格式修复和低置信 OCR/ASR 的风险修复可能继续增加时间。

“交付产物”只展示当前任务目录内存在的已知文件类型；质量报告里指向目录外的路径会被忽略。“打开输出目录”和“定位”不接收任意路径，只能由输出根目录、系列名、片名和已知产物键重新解析；空目录或没有 checkpoint、运行状态、质量报告及交付文件的普通目录不会被打开。“最近任务”只返回状态、复核数、产物数量和更新时间等摘要，加载具体任务后才读取完整产物清单。
...
合法同时对白单独记为 intentional overlap，不会被误判为失败。结构性错误会把任务标记为失败；阅读速度、识别置信度、同步关闭或模型保护等可人工判断的问题标记为“建议复核”，字幕文件仍保留。每项检查最多保存 20 个代表样本，其中约 60% 优先保留严重度/风险最高的候选，约 40% 覆盖完整时间轴；`sample_count` 和 `samples_truncated` 保留真实总量语义。前端按共同根因折叠同一字幕的重复候选，支持严重度、检查类型和逐条视听复核，不把抽样通过等同于整片通过。
